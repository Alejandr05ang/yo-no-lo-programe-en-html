...
async def test_admin_create_cohort_and_audit(auth_harness):
    ...
    res = await h.client.post("/api/admin/cohorts", json={"name": "Test", "slug": "test-c"}, headers=bearer("admin1"))
    assert res.status_code == 200
...
async def test_admin_change_active_session(auth_harness):
    ...
    # Create cohort
    res = await h.client.post("/api/admin/cohorts", json={"name": "Test2", "slug": "test-2"}, headers=bearer("admin1"))
    cohort_id = res.json()["id"]
    ...
    res = await h.client.patch(f"/api/admin/cohorts/{cohort_id}/active-session", json={"active_session_id": fake_session}, headers=bearer("admin1"))
    assert res.status_code == 200
...
async def test_admin_add_admin(auth_harness):
    ...
    res = await h.client.post("/api/admin/admins", json={"email": "newadmin@example.com"}, headers=bearer("admin1"))
    assert res.status_code == 200
    
    async with h.sessions() as session:
        a = await session.scalar(select(AdminAllowlist).where(AdminAllowlist.email == "newadmin@example.com"))

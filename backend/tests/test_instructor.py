...
async def setup_instructor_data(h):
    ...
    async with h.sessions.begin() as session:
        ...
        for u in users:
            if u.email == "i1@example.com": u1 = u
            if u.email == "i2@example.com": u2 = u
            if u.email == "s1@example.com": s1 = u
        ...
        session.add(CohortMembership(cohort_id=c1.id, user_id=u1.id, status="active"))
        session.add(CohortMembership(cohort_id=c2.id, user_id=u2.id, status="active"))
...
async def test_instructor_isolation(auth_harness):
    ...
    assert res3.json()["error"]["code"] == "FORBIDDEN"

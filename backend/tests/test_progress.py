...
async def setup_test_data(h):
    ...
    async with h.sessions.begin() as session:
        ...
        # Create sessions
        sess1 = SessionCatalog(day_number=1, order_index=1, title="S1", is_published=True)
        sess2 = SessionCatalog(day_number=2, order_index=2, title="S2", is_published=True)
...
async def test_progress_unlocked_and_locked(auth_harness):
    ...
    # Unlocked - PUT
    res = await h.client.put(f"/api/challenges/{data['c1']}/progress", json={"draft_code": "print(1)"}, headers=bearer("student"))
    assert res.status_code == 200
    assert res.json()["draft_code"] == "print(1)"
    
    # Locked - PUT
    res = await h.client.put(f"/api/challenges/{data['c2']}/progress", json={"draft_code": "print(1)"}, headers=bearer("student"))
    assert res.status_code == 403
...
async def test_progress_draft_persists_and_is_idempotent(auth_harness):
    ...
    res1 = await h.client.put(f"/api/challenges/{data['c1']}/progress", json={"draft_code": "v1"}, headers=bearer("student"))
    res2 = await h.client.put(f"/api/challenges/{data['c1']}/progress", json={"draft_code": "v2"}, headers=bearer("student"))
...
async def test_submission_unlocked_and_locked(auth_harness):
    ...
    # Locked - POST submit
    res = await h.client.post(f"/api/challenges/{data['c2']}/submit", json={"code_submitted": "print(1)"}, headers=bearer("student"))
    assert res.status_code == 403
    
    # Unlocked - POST submit
    res = await h.client.post(f"/api/challenges/{data['c1']}/submit", json={"code_submitted": "print(1)"}, headers=bearer("student"))
    assert res.status_code == 200
...
async def test_submission_concurrency(auth_harness):
    ...
    tasks = [
        h.client.post(f"/api/challenges/{data['c1']}/submit", json={"code_submitted": "v1"}, headers=bearer("student")),
        h.client.post(f"/api/challenges/{data['c1']}/submit", json={"code_submitted": "v2"}, headers=bearer("student"))
    ]
